...
if __name__ == "__main__":
    ...
    sign_time = 0
    for i in range(0,100):
        sign_time += test_sign(generateKeyPair(512)) # 512-bit Falcon algorithms is used in this project
    ...
    sign_r_time = 0
    for i in range(0, 100):
        sign_r_time += test_sign_recoverable(generateKeyPair(512)) # 512-bit Falcon algorithms is used in this project
    sign_r_time = sign_r_time / (100 * 1000)
    print("test_sign_recoverable(): ",
          float(Decimal(str(sign_r_time)).quantize(Decimal(".01"), rounding=ROUND_FLOOR)), " microseconds")

    recover_time = 0
    for i in range(0, 100):
        recover_time += test_recover(generateKeyPair(512)) # 512-bit Falcon algorithms is used in this project
    recover_time = recover_time / (100 * 1000)
    print("test_recover(): ",
          float(Decimal(str(recover_time)).quantize(Decimal(".01"), rounding=ROUND_FLOOR)), " microseconds")

    verify_time = 0
    for i in range(0, 100):
        verify_time += test_verify(generateKeyPair(512)) # 512-bit Falcon algorithms is used in this project
    ...
    verify_r_time = 0
    for i in range(0, 100):
        verify_r_time += test_verify_recoverable(generateKeyPair(512)) # 512-bit Falcon algorithms is used in this project
    verify_r_time = verify_r_time / (100 * 1000)
